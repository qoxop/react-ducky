...
const setPageKeyToUrl = (PageKey: string, url?: string | URL) => {
  if (url !== undefined && url !== null) {
    if (typeof url === 'string') {
      ...
      if (url.hash) {
        url.hash = url.hash + (/\?/.test(url.hash) ? `&${PageKey}` : `?${PageKey}`)
      } else {
        url.search = url.search + (/\?/.test(url.search) ? `&${PageKey}` : `?${PageKey}`)
      }
    }
  }
  return url
...
const enhanceHistory = () => {
  ...
  const updateActions = {
    pushState: (key: string) => {
      ...
      const position = PageStack.findIndex(key => key === lastKey);
      if (position > -1) {
  ...
  function enhanceHistoryMethod(methodName:'pushState'|'replaceState') {
    ...
    return function (data: any, unused: string = "", url?: string | URL) {
